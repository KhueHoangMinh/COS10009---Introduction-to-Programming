...
class GameWindow < Gosu::Window
  ...
  def update
    ...
    if Gosu.button_down?(Gosu::KbRight) && @shape_x <= WIDTH - SHAPE_DIM
        @shape_x += 3
    end
    if Gosu.button_down?(Gosu::KbLeft) && @shape_x >= 0
        @shape_x -= 3
    end
    if Gosu.button_down?(Gosu::KbUp) && @shape_y >= 0 
      @shape_y -= 3
    end
    if Gosu.button_down?(Gosu::KbDown) && @shape_y <= HEIGHT - SHAPE_DIM
      @shape_y += 3
    end
  end
