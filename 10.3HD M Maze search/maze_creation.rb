...
class GameWindow < Gosu::Window
  ...
  def search(cell_x ,cell_y)
    ...
    if (cell_x == ((MAP_WIDTH / CELL_DIM) - 1))
      ...
      if(@columns[cell_x][cell_y].north != nil && !@columns[cell_x][cell_y].north.visited && @columns[cell_x][cell_y].north.vacant)
        north_path = [[cell_x,cell_y-1]]
        count+=1
      end
      if(@columns[cell_x][cell_y].south != nil && !@columns[cell_x][cell_y].south.visited && @columns[cell_x][cell_y].south.vacant)
        south_path = [[cell_x,cell_y+1]]
        count+=1
      end
      if(@columns[cell_x][cell_y].east != nil && !@columns[cell_x][cell_y].east.visited && @columns[cell_x][cell_y].east.vacant)
        east_path =[[cell_x+1,cell_y]]
        count+=1
      end
      if(@columns[cell_x][cell_y].west != nil && !@columns[cell_x][cell_y].west.visited && @columns[cell_x][cell_y].west.vacant)
      ...
      # A path was found:
      if (path != nil)
        res = search(path[0][0],path[0][1])
        if(res != nil)
          return [[cell_x,cell_y]].concat(res)
        else
          return search(cell_x,cell_y)
        end
      else
        return nil
      end
