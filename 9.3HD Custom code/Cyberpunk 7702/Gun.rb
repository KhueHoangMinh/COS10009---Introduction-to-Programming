...
class Gun < Gosu::Window
    ...
    def fire(initposx, initposy, pointx, pointy)
        if @ready_to_fire == true
            ...
            when 'sniper'
                @shot_sound.play(0.5)
                @game.bullet << Bullet.new(@game, @angle_in_deg,initposx, initposy, @type, @sniperlength,60,rand(95..105),40,3000,true)
            when 'rifle'
                @shot_sound.play(0.1)
                @game.bullet << Bullet.new(@game, @angle_in_deg+2*rand(-0.2..0.2),initposx, initposy, @type, @otherslength,60,rand(10..40),20,2000,true)
            end
            @firesec=Process.clock_gettime(Process::CLOCK_MONOTONIC)
            @ready_to_fire = false
        end
